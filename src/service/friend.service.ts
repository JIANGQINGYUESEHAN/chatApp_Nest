...
export class FriendService {
    ...
    async checkRelation(OwnerId, FriendId) {
        ...
        let OwnerRelation = (await this.GetFriends(Owner.id)) as unknown as Array<any>
    ...
    async AddFriend(OwnerId, friendName) {
        let { Friend, Owner, IsFriend } = await this.checkRelation(OwnerId, friendName)
        if (IsFriend) { 
    ...
    async GetFriends(OwnerId) {
        const user = await this.friendRepository.createQueryBuilder('friends')
