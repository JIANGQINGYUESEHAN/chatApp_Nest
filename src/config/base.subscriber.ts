import { ObjectLiteral, InsertEvent, UpdateEvent, SoftRemoveEvent, RecoverEvent, TransactionStartEvent, TransactionCommitEvent, TransactionRollbackEvent, EventSubscriber, EntitySubscriberInterface, ObjectType, DataSource, LoadEvent, EntityTarget,} from "typeorm";
...
type SubscribeEvent<E extends ObjectLiteral> =
    ...
    export abstract class BaseSubscriber<E extends ObjectLiteral> implements EntitySubscriberInterface<E>{
      ...
      async  afterLoad(entity: any, event?: LoadEvent<E>) {
        ...
        return isNil(repository) ? this.getDataSource(event).getRepository(entity ?? this.entity) : getRepository<T, E>(this.getDataSource(event), repository)
    }
